              ...
              int ri, int rj, int rz, const Opm::CornerPointChopper& ch)
{
  ...
  ch.chop(istart, istart + settings.ilen, jstart, jstart + settings.jlen,
          zstart, zstart + settings.zlen, context, settings.resettoorigin);
...
#pragma omp parallel for schedule(static) reduction(+:finished_subsamples)
    ...
    if (settings.satnumvolumes) {
	for (int satnumidx = 0; satnumidx < 1; ++satnumidx) { // TODO!
