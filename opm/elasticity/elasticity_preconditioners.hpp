...
struct Schwarz {
  ...
  {
    return std::shared_ptr<type>(setup2(pre, post, target, zcells, op, gv, A, copy));
  }
  ...
  //! \param[in] gv The cornerpoint grid
  //! \param[out] thread Whether or not to clone for threads
  static type* setup2(int pre, int post, int target, int zcells,
                        std::shared_ptr<Operator>& op, const Dune::CpGrid& gv,
                        ASMHandler<Dune::CpGrid>& A, bool& copy);
};
...
struct AMG1 {
  ...
  typedef Dune::Amg::UnSymmetricCriterion<Matrix, CouplingMetric> CritBase;
...
struct AMG2Level {
                      ...
                      ASMHandler<Dune::CpGrid>& A, bool& copy)
  {
    ...
    Dune::shared_ptr<Schwarz::type> fsp(Schwarz::setup2(pre, post, target, zcells, op, gv, A, copy));
    copy = true;
